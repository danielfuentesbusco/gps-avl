<?php
	...
	$asignaciones = array();
	$q = "SELECT device_id FROM device_group_assign WHERE device_group_id = '{$_GET["id"]}';";
	$resultado = mysql_query($q);
	while ($row = mysql_fetch_assoc($resultado)){
		$asignaciones[$row["device_id"]] = true;
	}
  ...
  <body>  
    <div class="container">  
    	<div class="page-header" style="margin-top:0;">    
			<h3>Móviles <small>grupo <?php echo $row["name"];?></small></h3>
    	</div>
        <div class="row">
            <div class="col-md-12">
            	<div class="panel panel-default">
				  ...
				  <?php
					$q = "SELECT id, name FROM devices WHERE installed = '1' ORDER BY name;";
					$resultado = mysql_query($q);
					$i = 1;
					while ($row = mysql_fetch_assoc($resultado)) {
						if ($i % 6 == 0){
	...
	<script>
		<?php 
			if(isset($_POST["btn-submit"])){
				$query = "DELETE FROM device_group_assign WHERE device_group_id = '{$_POST["device_group_id"]}';";
				 mysql_query($query);
				foreach($_POST["checkbox"] as $key => $value){
					$query = "INSERT INTO device_group_assign (device_id, device_group_id) VALUES ('$key','{$_POST["device_group_id"]}');";
					 mysql_query($query);
				}
		?>
