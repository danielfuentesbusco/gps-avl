  ...
  <!-- Fixed navbar -->
    ...
    <div class="container">  
    	<div class="page-header" style="margin-top:0;">    
			<h3 style="margin-top:0;">Polígonos <small>Asignación de Rutas</small></h3>
    	</div>
        <div class="row">
            <div class="col-md-12">
				   ...
				   <table class="table table-hover" style="font-size: 100%;">
                        <thead> 
                            <tr>	
								...
								<th style="width: ;" nowrap>Estado</th>
								<th style="width: 70px;">
									...
									<ul class="dropdown-menu">
									  <li><a href="javascript:window.open('./asignar_ruta.php','verPoligono','width=900, height=720');">Asignación manual</a></li>
									  <li><a href="javascript:window.open('./asignar_archivo.php','verPoligono','width=900, height=720');">Subir archivo</a></li>
									</ul>
								  </div>
								</th>
                            </tr>
						...
						<?php
							...
							$q = "SELECT geofence_device.id, devices.name, geofences.name, geofences.tipo, geofence_device.fecha_inicio, geofence_device.fecha_termino, geofence_device.conductor_id FROM geofences, geofence_device, devices where geofences.tipo = 1 and geofence_device.geofence_id=geofences.id and devices.id=geofence_device.device_id;";
						...
						?>
							<tr>
								...
								<td style="vertical-align: middle;" nowrap><?php echo $row[2];?></td>
								<td style="vertical-align: middle;" nowrap>
									<?php
										$resultado2 = mysqli_query($link, "SELECT nombres FROM conductores where id = " . $row[6]) or die(mysqli_error());
										if ($row2 = mysqli_fetch_array($resultado2)) {
											echo $row2["nombres"];
										}
									?>
								</td>
								<td style="vertical-align: middle;" nowrap><?php
								$estado = "Activo";
								$fecha = strtotime($row[4]);
								echo date("d-m-Y h:i A",$fecha);
								echo " a "; 
								$fecha = strtotime($row[5]);
								if($fecha<time()){
									$estado = "Inactivo";
								}
								echo date("d-m-Y h:i A",$fecha);
								?></td>
								<td style="vertical-align: middle;" nowrap><?php echo $estado;?></td>
