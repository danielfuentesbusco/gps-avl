							...
							if ($row2 = mysql_fetch_array($resultado2)) {
								...
								while ($row = mysql_fetch_array($resultado)) {
									...
									if($row["address"]==null||$row["address"]==''){
										/*
										$q = "SELECT address FROM coordinate_address where latitude = '{$row["latitude"]}' AND longitude = '{$row["longitude"]}'";
										$resultado3 = mysql_query($q) or die(mysql_error());
										if ($row3 = mysql_fetch_array($resultado3)) {
											$row["address"] = utf8_encode($row3["address"]);
										}
										*/
										$row["address"] = getAddress($row["latitude"],$row["longitude"]);
										$query = "UPDATE location_report SET address = '{$row["address"]}' WHERE id = '{$row["id"]}';";
										mysql_query($query);
									}	
