  ...
  <!-- Fixed navbar -->
    ...
    <div class="container">  
    	<div class="page-header" style="margin-top:0;">    
			<h3 style="margin-top:0;">Reporte <small>Desconexión energía</small></h3>
    	</div>
        <div class="row">
            <div class="col-md-12">
	...
	<?php
		...
		$link = mysql_connect("sscl-db-instance.chapmhehekr9.us-east-1.rds.amazonaws.com",$username,$password) or die(mysql_error());
		mysql_select_db($database) or die(mysql_error());
		$q = "SELECT device_info.external_supply, device_info.utc, device_info.fecha 
				FROM device_info
				where device_id=$did  and utc >= '$fi' and utc <= '$ft' and utc is not null and utc <> ''  
				order by device_info.utc asc;";
		$resultado = mysql_query($q) or die(mysql_error());
		$arr = array();
		while ($row = mysql_fetch_array($resultado)) {
			$utc = date("d/m/Y h:i:s A",strtotime($row["utc"])-10800);
			$arr[] = "['{$utc}',{$row["external_supply"]}]";
		}
		echo implode(",",$arr);
		mysql_close($link);
		?>
        ...
        var options = {
          title: 'Reporte de desconexión energía',
          hAxis: {title: 'Fecha'},
          vAxis: {title: 'Conexión', minValue: 0, maxValue: 1},
          legend: 'none',
		  lineWidth: 0,
        };
